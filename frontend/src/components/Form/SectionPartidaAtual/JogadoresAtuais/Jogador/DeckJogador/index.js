...
const DeckJogador = ({ deck, index, jogadorIndex }) => {
  ...
  return (
    <DeckJogadorDiv>
      <input
        type="radio"
        id={`jogador${jogadorIndex}_atual${index}`}
        name={`jogador${jogadorIndex}_atual`}
        onChange={() => toggleAtual(jogadorIndex, index)}
        checked={atuais[jogadorIndex][index] ? "checked" : ""}
      ></input>
      {renderDeck(deck, index, jogadorIndex)}
      <ToggleButton
        toggleFunction={toggleBan}
      ...
      />
      <ToggleButton
        toggleFunction={toggleVitoria}
        jogador={jogadorIndex}
        posicao={index}
        matriz={vitorias}
        Simbolo1={<FaThumbsDown />}
        Simbolo2={<FaTrophy />}
      />
    </DeckJogadorDiv>
  );
